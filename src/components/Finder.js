...
class Finder extends React.Component {
  ...
  render() {
    const fighterList = this.state.unknownFighter.map(element => {
      return <Dragonball key={element.id} fighter={element} chooseFighter={this.props.chooseFighter} />
    })
    ...
    return (
      <div>
        ...
        <button className='button-3' onClick={() => this.refreshFighters()}> Summon New Fighters</button>
      </div>
    )
  }
}
